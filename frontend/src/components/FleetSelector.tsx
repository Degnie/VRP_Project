...
export function FleetSelector({ vehicleTypes, fleet, onChange }: Props) {
  ...
  const totalWeightKg = selected.reduce((sum, t) => sum + countFor(t.id) * t.weightCapacityKg, 0);
        ...
        {vehicleTypes.map((type) => (
          <div className="fleet-table-row" role="row" key={type.id}>
            ...
            <span className="fleet-vehicle-spec">
              {type.weightCapacityKg} kg · {type.volumeCapacityM3} m³
            </span>
            <input
      ...
      {totalVehicles > 0 && (
        <div className="fleet-summary">
          <p>
            ...
            {totalVehicles === 1 ? "" : "s"} · {totalWeightKg} kg combinados · {totalVolumeM3.toFixed(1)} m³ combinados
          </p>
